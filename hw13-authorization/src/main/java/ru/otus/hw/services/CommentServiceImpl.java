...
public class CommentServiceImpl implements CommentService {
    ...
    public List<CommentDto> findAllByBookId(long bookId) {
        return commentRepository.findAllByBookId(bookId).stream().map(dtoMapper::toCommentDto).toList();
    }

    @Transactional(readOnly = true)
    @Override
    public Optional<CommentDto> findCommentById(long id) {
        return commentRepository.findById(id).map(dtoMapper::toCommentDto);
    }
